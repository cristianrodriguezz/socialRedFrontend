...
import { likeOrDislikePhoto } from '../../utils/likeOrDislikePhoto'
import './Like.css'
import { useFetchCountLikes } from '../../hooks/fetchPhotos'


const Like = ({ likes, photoId }) => {
  const user = getUserFromLocalStorage()
  const { isLikePhoto, setIsLikePhoto } = useIsLikePhoto(likes, photoId)
  const { likeCount, loading } = useFetchCountLikes(photoId, isLikePhoto)

  const handleClick = (e) => {  
    likeOrDislikePhoto(user.id, photoId, e.target.checked)
